...
#    config.train_data = config.train_data + 'trainsfer_' + str(args.fold) + '.pth'
    ...
    if args.model_kind == 1:
        ...
        train_dataloader = DataLoader(train_dataset, collate_fn=my_collate_fn,
                                  batch_size=config.batch_size, shuffle=True, num_workers=3)
    else:
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=config.batch_size, num_workers=3)
...
def transfer_train(args):
    ...
    train_dataloader = DataLoader(train_dataset, collate_fn=my_collate_fn,
                                  batch_size=config.batch_size, shuffle=True, num_workers=3)
    import dataset
    val_dataset = dataset.ECGDataset(data_path=config.train_data, train=False, transfer=True)
...
def test(args):
    ...
    test_load = DataLoader(test_dataset, batch_size=64, num_workers=3,
                           shuffle=False)
    
    out = np.zeros((len(test_dataset), 55))
    print(len(test_dataset))
    for fold in range(5):
        model_save_dir = '%s/%s' % (config.ckpt+str(args.model_kind), config.model_name + '_' + str(fold))
        print(model_save_dir)
        model = getattr(models, 'myecgnet')()
...
def get_feature(args):
    ...
    for i in fold:
        ...
        model_save_dir = '%s/%s' % (config.ckpt + str(args.model_kind), config.model_name + '_' + str(i))
    #model_save_dir = '%s/%s' % (config.ckpt + str(args.model_kind),
    #                            config.model_name + '_' + str(args.fold))
#    utils.mkdirs(config.sub_dir)
    ...
    #config.test_label = config.train_label
...
#            output = torch.sigmoid(out)
        ...
        df = pd.DataFrame(out)
        df['filename'] = filename
        df.to_csv('lgb/DL_feature' + str(args.model_kind)+'/'+ args.model_name+ '_' + str(args.fold) + '.csv', index=None)

        out = prob_all.cpu().numpy()
        df = pd.DataFrame(out)
        df['filename'] = filename
        df.to_csv('lgb/DL_feature' + str(args.model_kind)+'/prod_'+ args.model_name+ '_' + str(args.fold) + '.csv', index=None)
