...
arrythmia = 'hf_round1_arrythmia.txt'
...
dl_name1 = 'myecgnet'
if add_ml_feature == 1:
    data2 = pd.read_csv('test_ml.csv',header=0, engine='python')
...
for i in fold:
    ...
    # col = col + ['idx']
    prob_dl += sigmoid(df[['prob1_' + str(i) for i in range(55)]].values)

    col = col1 + col2 + [str(ii) for ii in range(167)] + ['age', 'sex_new']
    ...
    for j in range(label_num):
        ...
        pred_fold[:, j] = clf.predict(val_data)
